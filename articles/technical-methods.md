...
Consider a trial with maximum sample size (N\_{}), planned endpoint time
(), and interim sample-size selection analyses after
...
to (N\_{}), `end_of_study` to (), and `interim_look` to ((n_1,,n_L)).

Let (Z_i ) denote treatment assignment for subject (i), matching the
package data column `treatment`: (Z_i = 1) for the treatment arm and
...
At the interim analysis held when (n\_) subjects have been enrolled, the
analysis calendar time is (E\_{n\_}). Subject (i)’s current observed
follow-up is
...
(u\_{i}), the subject is treated as censored at (u\_{i}). Subjects with
(i \> n\_) are not yet enrolled and contribute only to the
maximum-sample-size prediction.

At each interim analysis the design estimates two predictive
probabilities:

1.  (P\_{n\_}), the probability of final success if accrual stops at the
    current enrolled sample size and all enrolled subjects complete
    follow-up;
2.  (P\_{,}), the probability of final success if the trial continues to
    (N\_{}).

These are compared with thresholds (S\_) and (F\_), corresponding to the
package arguments `Sn` and `Fn`.
...
For treatment value (z ), where (z = 1) denotes the treatment arm and (z
= 0) denotes the control arm, interval (j) has constant hazard (\_{zj}).
...
where (s_J = ) and ({x}\_+ = (x,0)). The corresponding survival and
cumulative event probability are
...
evaluates (p_z()), and `haz_to_prop()` applies this transformation to
posterior hazard draws.

For observed follow-up ((T_i, \_i, Z_i)), the piecewise-exponential
...
Up to factors not involving (\_{zj}), the likelihood contribution for
treatment value (z) is
...
where (*z = (*,,\_{zJ})^). This sufficient-statistic form is what makes
the Gamma posterior update available in closed form.
...
where (\_0) is the shape and (\_0) is the rate. This follows the
...
treatment value (z), interval (j), and let (y\_{zj}) be the total
...
where (F(t) = 1 - S(t)). Equivalently, if (U (0,1)), then
...
Let (\_{}^{}) denote the data observed at look (), and let (^{}) denote
unobserved event times and future censoring indicators. The posterior
...
`impute_data()` draws completed outcomes conditional on those hazards,
and `test_stop_success()` repeats the impute-and-analyze cycle to
approximate the predictive probability by Monte Carlo simulation.

## 5. Interim decision algorithm

At interim look (), the package first estimates the posterior of the
hazard parameters from the currently observable data. It then uses Monte
Carlo integration to approximate (P\_{n\_}) and (P\_{,}).

Let (()) be the final success indicator for a completed analysis
dataset:
...
where (Q()) is the final analysis quantity and (c) is the success
threshold. In package notation, (c =) `prob_ha`. The final analysis
quantity (Q()) depends on the analysis method:

| Design setting | `method` | (Q()) | Supported alternatives |
|----|----|----|----|
| Two-arm randomized trial | `logrank` | (1-p()), where (p()) is the traditional log-rank test P-value, with one-sided variants defined in Section 6.1 | `"less"`, `"greater"`, `"two.sided"` |
| Two-arm randomized trial | `cox` | (1-p()), where (p()) is the traditional Wald-test P-value, with one-sided variants defined in Section 6.1 | `"less"`, `"greater"`, `"two.sided"` |
| Two-arm randomized trial | `chisq` | (1-p()), where (p()) is the traditional chi-square test P-value | `"two.sided"` |
| Two-arm randomized trial | `bayes-surv` | ((\< h_0 )) or ((\> h_0 )) | `"less"`, `"greater"` |
| Single-arm trial | `bayes-surv` | ((p_1() \< h_0 )) or ((p_1() \> h_0 )) | `"less"`, `"greater"` |
| Two-arm randomized trial | `bayes-bin` | (( \< h_0 )) or (( \> h_0 )) | `"less"`, `"greater"` |
| Single-arm trial | `bayes-bin` | ((\_1 \< h_0 )) or ((\_1 \> h_0 )) | `"less"`, `"greater"` |
...
0.025) rule. The value should be chosen during design calibration to
control the desired type I error rate across relevant null scenarios.

### 5.1 Predictive probability at the current sample size

The current-sample-size predictive probability (P\_{n\_}) is estimated
by:
...
where (*{n*}^{}) is the completed dataset formed from the (n\_) enrolled
subjects after imputing their remaining follow-up. Equivalently,
...
The maximum-sample-size predictive probability (P\_{,}) is estimated
similarly, except that the completed trial includes both currently
enrolled subjects and future subjects required to reach (N\_{}). For
each replicate, event times are imputed for the future subjects, the
completed dataset is analyzed, and success is recorded:
...
Thus the interim action at look () can be represented as
...
evidence” scale. For example, a one-sided test at (= 0.025) corresponds
to `prob_ha = 0.975`.

For the log-rank option, let (Z\_{}) denote the signed log-rank
statistic, with positive values corresponding to excess events in the
control arm under the package convention. Let (p\_{}) denote the
traditional two-sided log-rank test P-value. For the Cox option, let ()
be the estimated log hazard ratio for treatment versus control. The null
value `h0` is on the log-hazard-ratio scale, so the package uses
...
`h0 = log(margin)`. Here, lower treatment hazard corresponds to (Z\_{}
\< 0). Let (p\_{}) denote the two-sided Wald-test P-value relative to
`h0`. The package uses the following method-specific definitions of
(Q()):

| Method    | Alternative   | (Q())         |
|-----------|---------------|---------------|
| `logrank` | `"less"`      | ((Z\_{}))     |
| `logrank` | `"greater"`   | (1 - (Z\_{})) |
| `logrank` | `"two.sided"` | (1 - p\_{})   |
| `cox`     | `"less"`      | (1 - (Z\_{})) |
| `cox`     | `"greater"`   | ((Z\_{}))     |
| `cox`     | `"two.sided"` | (1 - p\_{})   |
| `chisq`   | `"two.sided"` | (1 - p\_{^2}) |

Here (p\_{^2}) is the traditional chi-square test P-value. The one-sided
directions differ between the log-rank and Cox rows because of the sign
convention of the package’s log-rank statistic.
...
cumulative event probabilities at (). In a two-arm design the treatment
effect is
...
where (p_1()) is the treatment-arm event probability and (p_0()) is the
control-arm event probability. The effect is on the event scale, not the
survival scale. For an adverse event, benefit usually means (\< 0).

Because (p_a() = 1 - ), posterior draws of () are obtained by
transforming posterior draws of (\_1) and (\_0):
...
In a single-arm design there is no (p_0()). The estimand becomes
(p_1()), and (h_0) is an external benchmark event probability. In
...
binary indicator of whether the endpoint has occurred by (). Subjects
with right-censored follow-up before () must be imputed or excluded
before this final test is applied, as described below.

Let (x_z) be the number of events and (n_z) the number of subjects in
treatment group (z). With `bin_prior = c(a, b)`, the event probability
in arm (z) has posterior distribution
...
probability. For an adverse binary event, benefit usually means (\_{} \<
0). With `alternative = "less"`, success is declared when
...
In a single-arm design, the estimand is (\_1) and `h0` is the external
benchmark event probability. Thus, `alternative = "less"` declares
success when ((\_1 \< h_0 ) \>) `prob_ha`.
...
Let (R = 1,,R\_{}) index simulated trials under a scenario (), where ()
denotes the data-generating parameters such as control hazard, treatment
hazard, accrual rate, loss-to-follow-up rate, and follow-up duration.
The trial-level random variables are:
...
Let (\_0) denote the null parameter space, i.e. the set of
...
uncertainty in both (*{n*}) and (\_{,}). The follow-up period after
accrual stops also affects operating characteristics because it
determines how much additional information is observed before the final
analysis.
...
The thresholds (S\_) and (F\_) may be constant across looks or may vary
by look. They interact with the final analysis threshold, number and
timing of looks, endpoint delay, accrual rate, loss to follow-up, prior
distribution, and maximum sample size.
...
The quantity `stop_and_fail` is particularly useful when tuning (S\_).
