...
`imputed_final = TRUE`, the package uses the selected complete-data test
directly if no outcomes are missing. When outcomes require imputation,
`riskdiff-wald` combines estimates and within-imputation variances using
Rubin’s rules. This requires at least two imputations and positive total
variance; zero total variance produces a non-estimability error.

Genuine final imputation is unsupported for `riskdiff-fm` because no
validated FM pooling rule is implemented. Simulations with FM and
`imputed_final = TRUE` require `prop_loss = 0` in both arms and reject
positive dropout at setup. The package does not substitute a pooled Wald
test for FM.

With nonzero `prop_loss`, the simulator draws dropout independently of
event time from an exponential distribution whose CDF at `end_of_study`
equals `prop_loss`. Actual censoring by dropout can be less frequent
because events can occur first. Independent censoring supports survival
inference, but complete-case binary analysis can still overestimate
event probabilities: early events remain observed while later endpoint
outcomes can be missing. Evaluate final imputation and sensitivity to
its event-time model for binary designs with dropout.
