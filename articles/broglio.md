...
#> # A tibble: 2 × 8
#>   scenario power stop_success stop_futility stop_max_N mean_N  sd_N
#>      <int> <dbl>        <dbl>         <dbl>      <dbl>  <dbl> <dbl>
#> 1        1 0.934        0.916         0.04      0.0440   171.  53.3
#> 2        2 0.068        0.056         0.846     0.098    210.  47.6
#> # ℹ 1 more variable: stop_and_fail <dbl>
```

The type I error under this design is slightly too large to be
considered acceptable. This was to be expected, since we kept the
$`P`$-value threshold as 0.05 despite having multiple interim looks.
However, we note that only simulated `N_trials = 500` trials, meaning if
the type I error was truly 0.05, then values in the interval
(`0.05 + c(-1, 1) * 1.96 * sqrt(0.05 * (1 - 0.05) / 500)`) would be
