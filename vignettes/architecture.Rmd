...
-   **`sim_comp_data()`**: Generates a complete trial dataset by calling `enrollment()`, `randomization()`, and `pwe_sim()`. Its `generation_cutpoints` argument defines the data-generating hazard partition.
-   **`survival_adapt()`**: Simulates a single adaptive trial. It passes `generation_cutpoints` to `sim_comp_data()`, uses `cutpoints` for posterior estimation and predictive imputation, and performs the final analysis via `test_final()`. With `return_trace = TRUE`, it also retains a compact audit trail for each completed interim look.
