...
\text{binding futility}, & P_{n_{\max},\ell} < F_\ell, \\
...
input_audit <- data.frame(
  ...
  `Source and mapping` = c(
    ...
    "Runtime example; the package's random censoring is not equivalent to the eight randomized patients who did not receive assigned treatment",
    "JAMA reports multiple imputation for incomplete outcomes; package implementation is an approximation",
    "Runtime choices; the sponsor's predictive draw count is unavailable",
    "Runtime choice; the sponsor briefing reports 10,000 simulated trials per scenario"
  ),
  check.names = FALSE
)
...
The following call simulates one trial under the descriptive FDA-rate scenario.
It uses only 100 predictive imputations so the vignette remains quick to build.
...
computed, its two fields are `NA` and `trial_success` remains `FALSE`.
...
With only 50 trials and 40 imputations per look, these estimates have substantial
Monte Carlo error. They demonstrate the workflow and the mutually exclusive
stopping categories; they do not estimate the trial's reported operating
characteristics. `summarise_sims()` retains the historical name `stop_success`
for stopping accrual for expected success and reports immediate success
separately as `stop_immediate_success`.
...
It does not reproduce the sponsor's source code, hierarchical hazard prior,
simulation scenarios, treatment-specific evaluation-window origins,
