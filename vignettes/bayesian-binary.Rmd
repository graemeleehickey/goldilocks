...
For the examples below, we use the default weakly informative Gamma$(0.1, 0.1)$ hazard prior and a uniform Beta$(1, 1)$ binary-endpoint prior:
...
Suppose the control event probability by 12 months is 35%, and the treatment is expected to reduce this to 25%. We use a beta-binomial final analysis with a uniform `Beta(1, 1)` prior in each arm. Since lower event probability is beneficial, we use `alternative = "less"` and compare the posterior distribution of
