...
-   `empty_interval = "prior"` leaves the interval with $d_{zj} = 0$ and $y_{zj} = 0$, so the posterior for that interval is exactly the specified Gamma prior.
...
Here $p_{\chi^2}$ is the traditional chi-square test P-value. The one-sided directions differ between the log-rank and Cox rows because of the sign convention of the package's log-rank statistic.
